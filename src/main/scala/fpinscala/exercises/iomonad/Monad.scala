...
trait Monad[F[_]] extends Functor[F]:
  ...
  extension [A](fa: F[A])
    ...
    def replicateM(n: Int): F[List[A]] = LazyList.fill(n)(fa).foldRight(unit(List[A]()))(_.map2(_)(_ :: _))
  ...
  def when[A](b: Boolean)(fa: => F[A]): F[Boolean] = if b then fa.as(true) else unit(false)
  ...
  def foldM_[A, B](l: LazyList[A])(z: B)(f: (B, A) => F[B]): F[Unit] = foldM(l)(z)(f).void

  def foreachM[A](l: LazyList[A])(f: A => F[Unit]): F[Unit] = foldM_(l)(())((u, a) => f(a).void)
...
object Monad:
  ...
  given function0Monad: Monad[Function0] with
    ...
    extension [A](fa: Function0[A]) def flatMap[B](f: A => Function0[B]) = () => f(fa())()
