...
trait Monad[F[_]] extends Applicative[F]:
  ...
  extension [A](fa: F[A])
    ...
    override def map2[B, C](fb: F[B])(f: (A, B) => C): F[C] = fa.flatMap(a => fb.map(b => f(a, b)))

  override def apply[A, B](ff: F[A => B])(fa: F[A]): F[B] = ff.flatMap(f => fa.map(f))
...
object Monad:
  ...
  def composeM[G[_], H[_]](using G: Monad[G], H: Monad[H], T: Traverse[H]): Monad[[x] =>> G[H[x]]] = new:
    ...
    extension [A](gha: G[H[A]]) override def flatMap[B](f: A => G[H[B]]): G[H[B]] = ???
