...
trait Traverse[F[_]] extends Functor[F], Foldable[F]:
  ...
  extension [A](fa: F[A])
    ...
    override def foldMap[B](f: A => B)(using mb: Monoid[B]): B = fa.traverse[Const[B, _], Nothing](f)
  ...
  def compose[G[_]: Traverse]: Traverse[[x] =>> F[G[x]]] = new:
    extension [A](fa: F[G[A]]) override def traverse[H[_]: Applicative, B](f: A => H[B]): H[F[G[B]]] = ???
...
object Traverse:
  ...
  given treeTraverse: Traverse[Tree] = new:
    extension [A](ta: Tree[A]) override def traverse[G[_]: Applicative, B](f: A => G[B]): G[Tree[B]] = ???
