      ...
      def map2[B, C](p2: Par[B])(f: (A, B) => C): Par[C] = es =>
        cb =>
          ...
          val combiner      = Actor[Either[A, B]](es):
            case Left(a)  => if br.isDefined then eval(es)(cb(f(a, br.get))) else ar = Some(a)
            case Right(b) => if ar.isDefined then eval(es)(cb(f(ar.get, b))) else br = Some(b)
          p(es)(a => combiner ! Left(a))
          p2(es)(b => combiner ! Right(b))

      def map[B](f: A => B): Par[B] = es => cb => p(es)(a => eval(es)(cb(f(a))))

      def flatMap[B](f: A => Par[B]): Par[B] = es => cb => p(es)(a => f(a)(es)(cb))
