...
object SimplePulls:
  ...
  enum Pull[+O, +R]:
    ...
    def toList: List[O] = fold(List.newBuilder[O])((bldr, o) => bldr += o)(1).result
    ...
    def uncons: Pull[Nothing, Either[R, (O, Pull[O, R])]] = Pull.done >> Result(step)
    ...
    def countViaMapAccumulate: Pull[Int, R] = Output(0) >> mapAccumulate(0)((s, o) => (s + 1, s + 1)).map(_(1))

    def tallyViaMapAccumulate[O2 >: O](using m: Monoid[O2]): Pull[O2, R] = Output(m.empty) >> mapAccumulate(m.empty):
      (s, o) =>
  ...
  object Pull:
    ...
    given [O]: Monad[[x] =>> Pull[O, x]] with
      ...
      extension [A](pa: Pull[O, A]) def flatMap[B](f: A => Pull[O, B]): Pull[O, B] = pa.flatMap(f)

    extension [O](self: Pull[O, Unit])

      def flatMapOutput[O2](f: O => Pull[O2, Unit]): Pull[O2, Unit] = self.uncons.flatMap:
        case Left(())        => Result(())
        case Right((hd, tl)) => f(hd) >> tl.flatMapOutput(f)

    val outputMonad: Monad[[x] =>> Pull[x, Unit]] = new:
      def unit[A](a: => A): Pull[A, Unit]                                                   = Output(a)
      extension [A](pa: Pull[A, Unit]) def flatMap[B](f: A => Pull[B, Unit]): Pull[B, Unit] = pa.flatMapOutput(f)
  ...
  object Stream:
    ...
    given Monad[Stream] with
      ...
      extension [A](sa: Stream[A]) def flatMap[B](f: A => Stream[B]): Stream[B] = sa.flatMapOutput(f)
...
object SimplePullExamples:
  ...
  def exists[I](f: I => Boolean): Pipe[I, Boolean] = src => src.map(f).toPull.tally(using Monoid.booleanOr).toStream
  ...
  def existsHalting[I](f: I => Boolean): Pipe[I, Boolean] = exists(f) andThen takeThrough(!_) andThen dropWhile(!_)
  ...
  def existsHalting2[I](f: I => Boolean): Pipe[I, Boolean] = exists(f) andThen takeThrough(!_) andThen last(false)

  def countGt40K[I]: Pipe[I, Boolean] = count andThen existsHalting(_ > 40000)

  def fromIterator[O](itr: Iterator[O]): Stream[O] = Pull
    .unfold(itr)(itr => if itr.hasNext then Right((itr.next(), itr)) else Left(itr)).void.toStream

  def processFile[A](file: java.io.File, p: Pipe[String, A])(using m: Monoid[A]): IO[A] = IO:
  ...
  val conversion: Pipe[String, Double] = trimmed andThen nonEmpty andThen nonComment andThen asDouble andThen
    convertToCelsius
