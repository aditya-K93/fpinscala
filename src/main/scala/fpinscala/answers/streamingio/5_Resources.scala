...
object Resources:
  ...
  final class Ref[F[_], A] private (underlying: AtomicReference[A], delay: [A] => (() => A) => F[A]):
  ...
  final class Scope[F[_]](parent: Option[Scope[F]], val id: Id, state: Ref[F, Scope.State[F]])(using F: MonadThrow[F]):
    ...
    def close: F[Unit] = state.modify:
      case State.Open(finalizer, subscopes) =>
        ...
        def go(rem: List[F[Unit]], error: Option[Throwable]): F[Unit] = rem match
    ...
    def findScope(target: Id): F[Option[Scope[F]]] = findThisOrSubScope(target).flatMap:
  ...
  enum Pull[+F[_], +O, +R]:
    ...
    case Handle(source: Pull[F, O, R], handler: Throwable => Pull[F, O, R]) extends Pull[F, O, R]
    ...
    def step[F2[x] >: F[x], O2 >: O, R2 >: R](scope: Scope[F2])(using F: MonadThrow[F2]): F2[StepResult[F2, O2, R2]] =
      ...
      this match
        ...
        case Handle(source, f)                         => source match
            case Handle(s2, g) => s2.handleErrorWith(x => g(x).handleErrorWith(y => f(y))).step(scope)
        ...
        case FlatMap(source, f)                        => source match
            case FlatMap(s2, g) => s2.flatMap(x => g(x).flatMap(y => f(y))).step(scope)
        ...
        case FlatMapOutput(source, f)                  => source.step(scope).flatMap:
            ...
            case Out(scope, hd, tl) => f(hd).flatMap(_ => tl.flatMapOutput(f)).step(scope)
        case OpenScope(source, finalizer)              => scope.open(finalizer.getOrElse(F.unit(())))
            .flatMap(subscope => WithScope(source, subscope.id, scope.id).step(subscope))
        case WithScope(source, scopeId, returnScopeId) => scope.findScope(scopeId)
            .map(_.map(_ -> true).getOrElse(scope -> false)).flatMap:
              case (newScope, closeAfterUse) => source.step(newScope).attempt.flatMap:
                  case Success(Out(scope, hd, tl)) => F.unit(Out(scope, hd, WithScope(tl, scopeId, returnScopeId)))
                  case Success(Done(outScope, r))  => scope.findScope(returnScopeId).map(_.getOrElse(outScope)).flatMap:
                      nextScope => scope.close.as(Done(nextScope, r))
                  case Failure(t)                  => scope.close.flatMap(_ => F.raiseError(t))

    def fold[F2[x] >: F[x], R2 >: R, A](init: A)(f: (A, O) => A)(using F: MonadThrow[F2]): F2[(R2, A)] =
      val scope                                                         = Scope.root[F2]
      def go(scope: Scope[F2], p: Pull[F2, O, R2], acc: A): F2[(R2, A)] = p.step(scope).flatMap:
    ...
    def toList[F2[x] >: F[x]: MonadThrow, O2 >: O]: F2[List[O2]] = fold(List.newBuilder[O])((bldr, o) => bldr += o)
      .map(_(1).result)
    ...
    def takeWhile(f: O => Boolean): Pull[F, O, Pull[F, O, R]] = uncons.flatMap:
      ...
      case Right((hd, tl)) => if f(hd) then Output(hd) >> tl.takeWhile(f) else Result(Output(hd) >> tl)

    def dropWhile(f: O => Boolean): Pull[F, Nothing, Pull[F, O, R]] = uncons.flatMap:
      case Left(r)         => Result(Result(r))
      case Right((hd, tl)) => if f(hd) then tl.dropWhile(f) else Result(Output(hd) >> tl)
    ...
    def filter(p: O => Boolean): Pull[F, O, R] = uncons.flatMap:
      case Left(r)         => Result(r)
      case Right((hd, tl)) => (if p(hd) then Output(hd) else Pull.done) >> tl.filter(p)
  ...
  object Pull:
    ...
    def unfoldEval[F[_], O, R](init: R)(f: R => F[Either[R, (O, R)]]): Pull[F, O, R] = Pull.Eval(f(init)).flatMap:
      case Left(r)        => Result(r)
    ...
    extension [F[_], R](self: Pull[F, Int, R])
      ...
      def slidingMean(n: Int): Pull[F, Double, R] =
        def go(window: collection.immutable.Queue[Int], p: Pull[F, Int, R]): Pull[F, Double, R] = p.uncons.flatMap:
          case Left(r)         => Result(r)
          case Right((hd, tl)) =>
    ...
    given [F[_], O]: Monad[[x] =>> Pull[F, O, x]] with
      ...
      extension [A](pa: Pull[F, O, A]) def flatMap[B](f: A => Pull[F, O, B]): Pull[F, O, B] = pa.flatMap(f)

    extension [F[_], O](self: Pull[F, O, Unit])

      def flatMapOutput[O2](f: O => Pull[F, O2, Unit]): Pull[F, O2, Unit] = FlatMapOutput(self, f)
  ...
  object Stream:
    ...
    def unfold[O, R](init: R)(f: R => Option[(O, R)]): Stream[Nothing1, O] = Pull.unfold(init)(r => f(r).toRight(r))
      .void

    def continually[O](o: O): Stream[Nothing1, O] = Pull.Output(o) >> continually(o)

    def iterate[O](initial: O)(f: O => O): Stream[Nothing1, O] = Pull.Output(initial) >> iterate(f(initial))(f)

    def eval[F[_], O](fo: F[O]): Stream[F, O] = Pull.Eval(fo).flatMap(Pull.Output(_))

    def unfoldEval[F[_], O, R](init: R)(f: R => F[Option[(O, R)]]): Stream[F, O] = Pull.Eval(f(init)).flatMap:
    ...
    def resource[F[_], R](acquire: F[R])(release: R => F[Unit]): Stream[F, R] = Pull.Eval(acquire)
    ...
    extension [F[_], O](self: Stream[F, O])
      ...
      def fold[A](init: A)(f: (A, O) => A)(using MonadThrow[F]): F[A] = self.fold(init)(f).map(_(1))
      ...
      def mapEval[O2](f: O => F[O2]): Stream[F, O2] = self.flatMapOutput(o => Stream.eval(f(o)))

      def handleErrorWith(handler: Throwable => Stream[F, O]): Stream[F, O] = Pull.Handle(self, handler)
    ...
    extension [O](self: Stream[Nothing, O])
      ...
      def fold[A](init: A)(f: (A, O) => A): A = (self: Stream[SyncTask, O]).fold(init)(f).resultOrThrow(1)
    ...
    given [F[_]]: Monad[[x] =>> Stream[F, x]] with
      ...
      extension [A](sa: Stream[F, A]) def flatMap[B](f: A => Stream[F, B]): Stream[F, B] = sa.flatMapOutput(f)
...
object ResourcesExample:
  ...
  def file(path: String): Stream[Task, Source] = Stream
    .resource(Task(Source.fromFile(path)))(s => Task { println("CLOSING"); s.close() })

  def lines(path: String): Stream[Task, String] = file(path)
    .flatMap(source => Stream.eval(Task(source.getLines)).flatMap(Stream.fromIterator))

  val printLines: Stream[Task, Unit] = lines("build.sbt").mapEval(line => Task(println(line)))
  ...
  def nonComment[F[_]]: Pipe[F, String, String] = src => src.filter(_.charAt(0) != '#')
  ...
  val conversion: Pipe[Task, String, String] = trimmed andThen nonEmpty andThen nonComment andThen asDouble andThen
    convertToCelsius andThen toString

  def convert(inputFile: String, outputFile: String): Task[Unit] = lines(inputFile).pipe(conversion)
    .pipe(writeLines(outputFile)).run

  def convertAll(inputFile: String, outputFile: String): Task[Unit] = lines(inputFile).flatMap(lines).pipe(conversion)
    .pipe(writeLines(outputFile)).run

  def convertMultisink(inputFile: String): Task[Unit] = lines(inputFile)
    .flatMap(file => lines(file).pipe(conversion).pipe(writeLines(file + ".celsius"))).run
