...
object ErrorHandling:
  ...
  enum Pull[+F[_], +O, +R]:
    ...
    case Handle[+F[_], +O, +R](source: Pull[F, O, R], handler: Throwable => Pull[F, O, R]) extends Pull[F, O, R]

    case Error(t: Throwable) extends Pull[Nothing, Nothing, Nothing]

    def step[F2[x] >: F[x], O2 >: O, R2 >: R](using F: MonadThrow[F2]): F2[Either[R2, (O2, Pull[F2, O2, R2])]] =
      this match
        ...
        case Handle(source, f)  => source match
            case Handle(s2, g) => s2.handleErrorWith(x => g(x).handleErrorWith(y => f(y))).step
    ...
    def fold[F2[x] >: F[x], R2 >: R, A](init: A)(f: (A, O) => A)(using F: MonadThrow[F2]): F2[(R2, A)] = step.flatMap:
  ...
  object Pull:
    ...
    extension [F[_], O](self: Pull[F, O, Unit])
      ...
      def flatMapOutput[O2](f: O => Pull[F, O2, Unit]): Pull[F, O2, Unit] = self.uncons.flatMap:
        case Left(())        => Result(())
