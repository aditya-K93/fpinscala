...
object Nonblocking:
  ...
  object Par:
      ...
      def map2[B, C](p2: Par[B])(f: (A, B) => C): Par[C] = es =>
        cb =>
          ...
          val combiner      = Actor[Either[A, B]](es):
            case Left(a)  => if br.isDefined then Par.eval(es)(cb(f(a, br.get))) else ar = Some(a)
            case Right(b) => if ar.isDefined then Par.eval(es)(cb(f(ar.get, b))) else br = Some(b)
          p(es)(a => combiner ! Left(a))
          p2(es)(b => combiner ! Right(b))

    extension [A](p: Par[A]) def map[B](f: A => B): Par[B] = es => cb => p(es)(a => eval(es)(cb(f(a))))
    ...
    def choiceMap[K, V](key: Par[K])(choices: Map[K, Par[V]]): Par[V] = es => cb => key(es)(k => choices(k)(es)(cb))

    /* `chooser` is usually called `flatMap` or `bind`. */
    def chooser[A, B](p: Par[A])(f: A => Par[B]): Par[B] = p.flatMap(f)

    def choiceViaFlatMap[A](p: Par[Boolean])(f: Par[A], t: Par[A]): Par[A] = p.flatMap(b => if b then t else f)

    def choiceNViaFlatMap[A](p: Par[Int])(choices: List[Par[A]]): Par[A] = p.flatMap(i => choices(i))
