...
object Par:
  ...
  def sequence[A](pas: List[Par[A]]): Par[List[A]] = sequenceBalanced(pas.toIndexedSeq).map(_.toList)
  ...
  extension [A](pa: Par[A]) def flatMapViaJoin[B](f: A => Par[B]): Par[B] = join(pa.map(f))
