...
object IO2a:
  ...
  val composite: IO[Unit] = actions.foldLeft(IO(()))((acc, a) => acc.flatMap(_ => a))
...
object IO2bTests:
  ...
  val g: Int => TailRec[Int] = List.fill(10000)(f).foldLeft(f)((a, b) => x => TailRec.suspend(a(x).flatMap(b)))
...
object BuiltInTailCalls:
  ...
  val g: Int => TailRec[Int] = List.fill(10000)(f).foldLeft(f)((a, b) => x => tailcall(a(x).flatMap(b)))
...
object IO2c:
  ...
  object Async:
    ...
    def suspend[A](aa: => Async[A]): Async[A] = Suspend(Par.delay(aa)).flatMap(identity)
...
object IO3:
  ...
  enum Free[F[_], A]:
    ...
    def flatMapU[G[_], B](f: A => Free[G, B]): Free[[x] =>> F[x] | G[x], B] = FlatMap(union, a => f(a).union)
    ...
    def translate[G[_]](fToG: [x] => F[x] => G[x]): Free[G, A] = runFree([x] => (fx: F[x]) => Suspend(fToG(fx)))
  ...
  enum Console[A]:
    ...
    def toState: ConsoleState[A] = this match
      ...
      case PrintLine(line) => ConsoleState(bufs => ((), bufs.copy(out = bufs.out :+ line))) // append to the output
  ...
  extension [A](fa: Free[Console, A])
    ...
    def unsafeRunConsole: A = fa.translate([x] => (c: Console[x]) => c.toThunk).runTrampoline
  ...
  extension [A](fa: Free[Console, A])
    ...
    def toReader: ConsoleReader[A] = fa.runFree([x] => (c: Console[x]) => c.toReader)

    def toState: ConsoleState[A] = fa.runFree([x] => (c: Console[x]) => c.toState)
  ...
  def read(
    ...
    fromPosition: Long,
    numBytes: Int
  ): Free[Par, Either[Throwable, Array[Byte]]] = Suspend(
...
object IO4:
  ...
  object Files:
    ...
    def readLines(file: String): Free[Files, List[String]] = Free.Suspend(Files.ReadLines(file))

  def cat(file: String): Free[[x] =>> Files[x] | Console[x], Unit] = Files.readLines(file).flatMap: lines =>
    Console.printLn(lines.mkString("\n"))

  extension [A](fa: Free[[x] =>> Files[x] | Console[x], A])

    def toThunk: () => A = fa.runFree([x] =>
...
object IO5:
  ...
  def cat[F[_]](file: String)(using c: Console[F], f: Files[F], m: Monad[F]): F[Unit] = f.readLines(file).flatMap:
    lines => c.printLn(lines.mkString("\n"))
