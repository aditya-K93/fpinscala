...
object Prop:
  ...
  def forAll[A](g: Int => Gen[A])(f: A => Boolean): Prop = (max, n, rng) =>
    ...
    val props: List[Prop] = LazyList.from(0).take(max.toInt + 1).map(i => forAll(g(i))(f)).toList
  ...
  def forAllPar[A](g: Gen[A])(f: A => Par[Boolean]): Prop = forAll(executors ** g)((s, a) => f(a).run(s).get)

  def verifyPar(p: Par[Boolean]): Prop = forAllPar(Gen.unit(()))(_ => p)

  def forAllPar2[A](g: Gen[A])(f: A => Par[Boolean]): Prop = forAll(executors ** g)((s, a) => f(a).run(s).get)

  def forAllPar3[A](g: Gen[A])(f: A => Par[Boolean]): Prop = forAll(executors ** g):
    case s ** a => f(a).run(s).get

  val gpy: Gen[Par[Int]] = Gen.choose(0, 10).map(Par.unit(_))
  val p4                 = forAllPar(gpy)(py => equal(py.map(y => y), py))

  lazy val gpy2: Gen[Par[Int]] = choose(-100, 100).listOfN(choose(0, 20))
...
object Gen:
  ...
  def cartesian[A](s: LazyList[LazyList[A]]): LazyList[LazyList[A]] = s
    .foldRight(LazyList(LazyList[A]()))((hs, ts) => map2LazyList(hs, ts)(LazyList.cons(_, _)))
  ...
  def sequenceOption[A](o: List[Option[A]]): Option[List[A]] = o
    .foldLeft[Option[List[A]]](Some(List()))((t, h) => map2Option(h, t)(_ :: _)).map(_.reverse)
  ...
  def union[A](g1: Gen[A], g2: Gen[A]): Gen[A] =
    Gen(State(RNG.boolean).flatMap(b => if b then g1.sample else g2.sample), interleave(g1.exhaustive, g2.exhaustive))

  def interleave[A](s1: LazyList[A], s2: LazyList[A]): LazyList[A] = s1.map(Some(_)).zipAll(s2.map(Some(_)), None, None)
  ...
  def weighted[A](g1: (Gen[A], Double), g2: (Gen[A], Double)): Gen[A] =
    ...
    def bools: LazyList[Boolean] = randomLazyList(double.map(_ < g1Threshold))(RNG.Simple(302837L))

    Gen(
      State(RNG.double).flatMap(d => if d < g1Threshold then g1._1.sample else g2._1.sample),
      interleave(bools, g1._1.exhaustive, g2._1.exhaustive)
    )

  /* Produce an infinite random lazy list from a `Gen` and a starting `RNG`. */
  def randomLazyList[A](g: Gen[A])(rng: RNG): LazyList[A] = LazyList.unfold(rng)(rng => Some(g.sample.run(rng)))
...
object Cogen:
  ...
  def takeWhilePropInt = forAll(Gen.int.list ** fn(cogenInt, Gen.boolean).unsized)((ys, f) => ys.takeWhile(f).forall(f))
