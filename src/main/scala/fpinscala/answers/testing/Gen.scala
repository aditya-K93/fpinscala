...
object Prop:
  ...
  def randomLazyList[A](g: Gen[A])(rng: RNG): LazyList[A] = LazyList.unfold(rng)(rng => Some(g.run(rng)))
  ...
  val gpy2: Gen[Par[Int]] = choose(-100, 100).listOfN(choose(0, 20))
    .map(ys => ys.foldLeft(Par.unit(0))((p, y) => Par.fork(p.map2(Par.unit(y))(_ + _))))

  extension [A](self: List[A])

    def parTraverse[B](f: A => Par[B]): Par[List[B]] = self
      .foldRight(Par.unit(Nil: List[B]))((a, pacc) => Par.fork(f(a).map2(pacc)(_ :: _)))

  val gpy3: Gen[Par[Int]] = choose(-100, 100).listOfN(choose(0, 20)).map(ys => ys.parTraverse(Par.unit).map(_.sum))
...
object Gen:
  ...
  def choose(start: Int, stopExclusive: Int): Gen[Int] = State(RNG.nonNegativeInt)
  ...
  def choose(i: Double, j: Double): Gen[Double] = State(RNG.double).map(d => i + d * (j - i))
...
object Cogen:
  ...
  def fn[A, B](in: Cogen[A], out: Gen[B]): Gen[A => B] = State[RNG, A => B]: rng =>
