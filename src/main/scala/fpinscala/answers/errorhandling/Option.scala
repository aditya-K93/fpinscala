...
enum Option[+A]:
  ...
  def filter_1(f: A => Boolean): Option[A] = flatMap(a => if f(a) then Some(a) else None)
...
object Option:
  ...
  def mean(xs: Seq[Double]): Option[Double] = if xs.isEmpty then None else Some(xs.sum / xs.length)
