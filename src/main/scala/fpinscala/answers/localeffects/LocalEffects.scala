...
object STArray:
  ...
  def apply[S, A: ClassTag](sz: Int, v: A): ST[S, STArray[S, A]] = ST(new STArray[S, A](Array.fill(sz)(v)))

  def fromList[S, A: ClassTag](xs: List[A]): ST[S, STArray[S, A]] = ST(new STArray[S, A](xs.toArray))
...
object STMap:
  ...
  def fromMap[S, K, V](m: Map[K, V]): ST[S, STMap[S, K, V]] = ST(new STMap((HashMap.newBuilder ++= m).result()))
