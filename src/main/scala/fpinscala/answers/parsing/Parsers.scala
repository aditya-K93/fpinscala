...
trait Parsers[Parser[+_]]:
   ...
   */
  def doubleString: Parser[String] = regex("[-+]?([0-9]*\\.)?[0-9]+([eE][-+]?[0-9]+)?".r).token
  ...
  extension [A](p: Parser[A])
    ...
    def listOfN(n: Int): Parser[List[A]] = if n <= 0 then succeed(Nil) else p.map2(p.listOfN(n - 1))(_ :: _)

    def map[B](f: A => B): Parser[B] = p.flatMap(f andThen succeed)

    def map2[B, C](p2: => Parser[B])(f: (A, B) => C): Parser[C] = p.product(p2).map((a, b) => f(a, b))
    ...
    def product[B](p2: => Parser[B]): Parser[(A, B)] = p.flatMap(a => p2.map(b => (a, b)))
...
case class ParseError(stack: List[(Location, String)] = Nil):
  ...
  def collapseStack(s: List[(Location, String)]): List[(Location, String)] = s.groupBy(_._1).view
    .mapValues(_.map(_._2).mkString("; ")).toList.sortBy(_._1.offset)
