...
object Sliceable extends Parsers[Sliceable.Parser]:
  ...
  extension [A](p: Parser[A])
    ...
    def run(s: String): Either[ParseError, A] = p(ParseState(Location(s), false)).extract(s)
    ...
    * Note that this implementation is less efficient than it could
    ...
    def flatMap[B](f: A => Parser[B]): Parser[B] = s =>
      p(s.unslice) match
        case Success(a, n)     => f(a)(s.advanceBy(n).reslice(s)).addCommit(n != 0).advanceSuccess(n)
        case Slice(n)          => f(s.slice(n))(s.advanceBy(n).reslice(s)).advanceSuccess(n)
        case f @ Failure(_, _) => f
      ...
      p(s) match
        ...
        case Slice(n)          =>
          ...
          p2(s2) match
            ...
            case Slice(m)          =>
              if s.isSliced then Slice(n + m).asInstanceOf[Result[C]] else Success(f(s.slice(n), s2.slice(m)), n + m)
            case f @ Failure(_, _) => f
        case f @ Failure(_, _) => f
