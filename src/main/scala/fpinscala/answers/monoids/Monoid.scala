...
object Monoid:
  ...
  def monoidLaws[A](m: Monoid[A], gen: Gen[A]): Prop =
    val associativity = Prop.forAll(gen ** gen ** gen):
      case a ** b ** c => m.combine(a, m.combine(b, c)) == m.combine(m.combine(a, b), c)
    .tag("associativity")
  ...
  def foldRight[A, B](as: List[A])(acc: B)(f: (A, B) => B): B = foldMap(as, dual(endoMonoid))(f.curried)(acc)

  // Folding to the left is the same except we flip the arguments to
  // the function `f` to put the `B` on the correct side.
  def foldLeft[A, B](as: List[A])(acc: B)(f: (B, A) => B): B = foldMap(as, endoMonoid)(a => b => f(b, a))(acc)
  ...
  def parFoldMap[A, B](as: IndexedSeq[A], m: Monoid[B])(f: A => B): Par[B] = Par.parMap(as)(f).flatMap: bs =>
  ...
  def ordered(ints: IndexedSeq[Int]): Boolean = foldMapV(ints, orderedMonoid)(i => Some(Interval(true, i, i)))
  ...
  val wcMonoid: Monoid[WC] = new Monoid[WC]:
    ...
    def combine(wc1: WC, wc2: WC) = (wc1, wc2) match
      ...
      case (WC.Part(l, w, r), WC.Part(l2, w2, r2)) => WC.Part(l, w + (if (r + l2).isEmpty then 0 else 1) + w2, r2)
  ...
  def count(s: String): Int =
    ...
    def wc(c: Char): WC   = if c.isWhitespace then WC.Part("", 0, "") else WC.Stub(c.toString)
  ...
  def foldMapG[A, B](as: List[A])(f: A => B)(using m: Monoid[B]): B = foldMap(as, m)(f)

  // Variant of foldMapV which takes Monoid[B] as a context param
  def foldMapVG[A, B](as: IndexedSeq[A])(f: A => B)(using m: Monoid[B]): B = foldMapV(as, m)(f)
