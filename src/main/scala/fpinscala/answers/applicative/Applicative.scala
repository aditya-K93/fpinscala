...
trait Applicative[F[_]] extends Functor[F]:
  ...
  extension [A](fa: F[A])
    ...
    def map2[B, C](fb: F[B])(f: (A, B) => C): F[C] = apply(apply(unit(f.curried))(fa))(fb)
  ...
  extension [A](fa: F[A])
    ...
    def map3[B, C, D](fb: F[B], fc: F[C])(f: (A, B, C) => D): F[D] = apply(apply(apply(unit(f.curried))(fa))(fb))(fc)
  ...
  def compose[G[_]](G: Applicative[G]): Applicative[[x] =>> F[G[x]]] = new:
    ...
    extension [A](fga: F[G[A]])
      override def map2[B, C](fgb: F[G[B]])(f: (A, B) => C) = self.map2(fga)(fgb)(G.map2(_)(_)(f))
...
object Applicative:
  ...
  object ZipList:
    ...
    given zipListApplicative: Applicative[ZipList] with
      ...
      extension [A](fa: ZipList[A])
        ...
        override def map2[B, C](fb: ZipList[B])(f: (A, B) => C) = fa.zip(fb).map(f.tupled)
  ...
  object WebFormExample:
    ...
    def validName(name: String): Validated[List[String], String] =
      if name != "" then Validated.Valid(name) else Validated.Invalid(List("Name cannot be empty"))
    ...
    def validateWebForm(name: String, birthdate: String, phone: String): Validated[List[String], WebForm] =
      validName(name).map3(validBirthdate(birthdate), validPhone(phone))(WebForm(_, _, _))
  ...
  object WebFormExampleWithNonEmptyList:
    ...
    object NonEmptyList:
      ...
      given nelSemigroup[A]: Semigroup[NonEmptyList[A]] with
        ...
        def combine(x: NonEmptyList[A], y: NonEmptyList[A]) = NonEmptyList(x.head, x.tail ++ (y.head :: y.tail))

    case class WebForm(name: String, birthdate: LocalDate, phoneNumber: String)

    def validName(name: String): Validated[NonEmptyList[String], String] =
      if name != "" then Validated.Valid(name) else Validated.Invalid(NonEmptyList("Name cannot be empty"))
  ...
  given stateMonad[S]: Monad[State[S, _]] with
    ...
    extension [A](st: State[S, A])
      ...
      override def flatMap[B](f: A => State[S, B]): State[S, B] = State.flatMap(st)(f)
