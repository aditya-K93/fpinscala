...
object List: // `List` companion object. Contains functions for creating and working with lists.
  ...
  def incrementEach(l: List[Int]): List[Int] = foldRight(l, Nil: List[Int], (i, acc) => Cons(i + 1, acc))
  ...
  def filterViaFlatMap[A](l: List[A], f: A => Boolean): List[A] = flatMap(l, a => if f(a) then List(a) else Nil)
