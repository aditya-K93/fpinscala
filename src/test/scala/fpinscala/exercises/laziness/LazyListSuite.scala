...
class LazyListSuite extends PropSuite:
  ...
  test("LazyList.take")(genSmallInt ** genLazyList):
    case n ** lazyList => assertEquals(lazyList.take(n).toList, lazyList.toList.take(n))

  test("LazyList.drop")(genSmallInt ** genLazyList):
    case n ** lazyList => assertEquals(lazyList.drop(n).toList, lazyList.toList.drop(n))

  test("LazyList.takeWhile")(genSmallInt ** genLazyList):
    case n ** lazyList => assertEquals(lazyList.takeWhile(_ != n).toList, lazyList.toList.takeWhile(_ != n))

  test("LazyList.forAll")(genSmallInt ** genLazyList):
    case n ** lazyList => assertEquals(lazyList.forAll(_ != n), !lazyList.toList.contains(n))
  ...
  test("LazyList.unfold")(genMidInt): n =>
    val genFirstNumbers: Int => Option[(Int, Int)] = m => if m > n then None else Some((m, m + 1))
  ...
  test("LazyList.fromViaUnfold")(genMidInt ** genMidInt):
    case n ** a => assertEquals(fromViaUnfold(a).take(n).toList, (a until (a + n)).toList)

  test("LazyList.continuallyViaUnfold")(genMidInt ** genMidInt):
    case n ** a => assertEquals(continuallyViaUnfold(a).take(n).toList, List.fill(n)(a))
