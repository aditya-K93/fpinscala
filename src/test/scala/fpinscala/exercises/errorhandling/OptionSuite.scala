...
class OptionSuite extends PropSuite:
  ...
  private val genIntOption: Gen[Option[Int]] = Gen.union(Gen.unit(None), Gen.int.map(Some(_)))
  ...
  private val genListWithoutNone: Gen[List[Option[Int]]] = genList(Gen.int.map(Some(_)))

  private val genOptionSeq: Gen[List[Option[Int]]] = Gen
    .union(genNoneSeq, Gen.union(genListWithNone, genListWithoutNone))

  private val genListWithRandomString: Gen[List[String]] = genList(Gen.union(Gen.unit("one"), Gen.int.map(_.toString)))

  private val genListWithValidNumbers: Gen[List[String]] = genList(Gen.int.map(_.toString))

  private val genStringList: Gen[List[String]] = Gen.union(genListWithRandomString, genListWithValidNumbers)
  ...
  test("Option.sequence")(genOptionSeq): optionList =>
    val expected: Option[List[Int]] =
      if optionList.contains(None) then None else Some(optionList.flatMap(_.map(List(_)).getOrElse(List.empty[Int])))
    assertEquals(Option.sequence(optionList), expected)

  test("Option.traverse")(genStringList): list =>
    val expected: Option[List[Int]] = if list.contains("one") then None else Some(list.flatMap(_.toIntOption))
