...
class EitherSuite extends PropSuite:
  ...
  private val genEither: Gen[Either[String, Int]] = Gen.union(genString.map(Left(_)), Gen.int.map(Right(_)))
  ...
  test("Either.flatMap")(genEither): either =>
    val f: Int => Either[String, Int] = n => if n % 2 == 0 then Right(n / 2) else Left("An odd number")
  ...
  object Name:
    ...
    def make(name: String): Either[String, Name] =
      if name == "" || name == null then Left("Name is empty.") else Right(Name(name))

    def make2(name: String): Either[List[String], Name] =
      if name == "" || name == null then Left(List("Name is empty.")) else Right(Name(name))

  case class Age(value: Int)

  object Age:

    def make(age: Int): Either[String, Age] = if age < 0 then Left("Age is out of range.") else Right(Age(age))
  ...
  object Person:
    ...
    def make(name: String, age: Int): Either[String, Person] = Name.make(name).map2(Age.make(age))(Person(_, _))
  ...
  test("Either.traverse")(genAgesList): ageList =>
    val expected = if ageList.exists(_ < 0) then Left("Age is out of range.") else Right(ageList.map(Age(_)))
  ...
  test("Either.traverseAll")(genAgesList): ageList =>
    ...
    val expected =
      if negCount > 0 then Left(List.fill(negCount)("Age is out of range.")) else Right(ageList.map(Age(_)))
