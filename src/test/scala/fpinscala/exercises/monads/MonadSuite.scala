...
object MonadSuite extends Assertions:
  ...
  private trait TestedMonad[F[_]]:
    ...
    def assertFs[A](actual: F[A], expected: F[A]): Unit = Assertions.assertEquals(actual, expected)
  ...
  private val parMonad: TestedMonad[Par[_]] = new TestedMonad[Par]:
    ...
    override def assertFs[A](actual: Par[A], expected: Par[A]): Unit = Assertions
      .assertEquals(actual.run(service).get(), expected.run(service).get())

  private def parserMonad(s: String): TestedMonad[Parser[_]] = new TestedMonad[Parser]:
    val monad: Monad[Parser]                                               = Monad.parserMonad(UnitTestParser)
    def pure[A]: A => Parser[A]                                            = succeed
    override def assertFs[A](actual: Parser[A], expected: Parser[A]): Unit = Assertions
      .assertEquals(actual.run(s), expected.run(s))
  ...
  private val lazyListMonad: TestedMonad[LazyList[_]] = new TestedMonad[LazyList]:
  ...
  private val readerMonad: TestedMonad[Reader[Unit, _]] = new TestedMonad[Reader[Unit, _]]:
    val monad: Monad[Reader[Unit, _]]                                                  = Reader.readerMonad[Unit]
    def pure[A]: A => Reader[Unit, A]                                                  = monad.unit
    override def assertFs[A](actual: Reader[Unit, A], expected: Reader[Unit, A]): Unit = Assertions
