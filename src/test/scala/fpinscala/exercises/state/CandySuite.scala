...
class CandySuite extends PropSuite:
  ...
  private val genInput: Gen[Input] = Gen.boolean.map(b => if b then Coin else Turn)
  ...
  test("Candy: spend some coins")(genLockedMachine ** genPosInt):
    case machine ** myCoins =>
      ...
      val ((coins, candies), machine1): ((Int, Int), Machine) = simulateMachine(wantToSpendAllMyCoins).run(machine)
