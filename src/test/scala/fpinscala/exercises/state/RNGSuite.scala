...
class RNGSuite extends PropSuite:
  ...
  test("RNG.intDouble")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(rng, counter, intDouble, (_, d) => isInInterval(d)))

  test("RNG.doubleInt")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(rng, counter, doubleInt, (d, _) => isInInterval(d)))
  ...
  test("RNG.map")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(rng, counter, map(int)(_.toString), _.toIntOption.isDefined))
  ...
  test("RNG.mapViaFlatMap")(genRNG ** genCounter):
    case rng ** counter => assert(checkRND(rng, counter, mapViaFlatMap(int)(_.toString), _.toIntOption.isDefined))
  ...
  private def checkRND[A](rng: RNG, counter: Int, generate: Rand[A], isCorrect: A => Boolean): Boolean =
    @tailrec
    def loop(rng: RNG, counter: Int, previousValue: Option[A] = None): Boolean = counter < 0 || {
