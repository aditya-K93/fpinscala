...
class ListSuite extends PropSuite:
  ...
  test("List.drop")(genIntList ** genSmallNum):
    case list ** n => assertEquals(List.drop(list, n), scalaListToList(listToScalaList(list).drop(n)))
  ...
  test("List.concat")(genListOfLists): listOfLists =>
    assertEquals(List.concat(listOfLists), scalaListToList(listToScalaList(listOfLists).flatMap(listToScalaList)))

  test("List.incrementEach")(genIntList): list =>
    assertEquals(List.incrementEach(list), scalaListToList(listToScalaList(list).map(_ + 1)))

  test("List.doubleToString")(genDoubleList): list =>
    assertEquals(List.doubleToString(list), scalaListToList(listToScalaList(list).map(_.toString)))

  test("List.map")(genIntList): list =>
    assertEquals(List.map(list, _ * 2), scalaListToList(listToScalaList(list).map(_ * 2)))

  test("List.filter")(genIntList): list =>
    assertEquals(List.filter(list, _ % 2 == 0), scalaListToList(listToScalaList(list).filter(_ % 2 == 0)))

  test("List.flatMap")(genIntList): list =>
    assertEquals(List.flatMap(list, a => List(a, a)), scalaListToList(listToScalaList(list).flatMap(a => SList(a, a))))

  test("List.filterViaFlatMap")(genIntList): list =>
    assertEquals(List.filterViaFlatMap(list, _ % 2 == 0), scalaListToList(listToScalaList(list).filter(_ % 2 == 0)))
  ...
  test("random lists - List.hasSubsequence")(genIntList ** genIntList):
    case list1 ** list2 =>
      assertEquals(List.hasSubsequence(list1, list2), listToScalaList(list1).containsSlice(listToScalaList(list2)))
