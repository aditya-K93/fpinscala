...
class JSONSuite extends PropSuite:
  ...
  test("JSON.JNull")(Gen.unit(())): _ =>
    ...
    assertEquals(parser.run("""[ null, null, null ]"""), Right(JArray(IndexedSeq(JNull, JNull, JNull))))
  ...
  test("JSON.JBool")(Gen.boolean ** Gen.boolean ** Gen.boolean):
    case b1 ** b2 ** b3 =>
      ...
      assertEquals(parser.run(s"""[ $b1, $b2, $b3 ]"""), Right(JArray(IndexedSeq(JBool(b1), JBool(b2), JBool(b3)))))
  ...
  test("JSON.JArray")(Gen.double ** genString ** Gen.boolean):
    case d ** s ** b =>
      ...
      assertEquals(
        parser.run(s"""[ null, [ $d, [ "$s", $b ] ] ]"""),
        Right(JArray(IndexedSeq(JNull, JArray(IndexedSeq(JNumber(d), JArray(IndexedSeq(JString(s), JBool(b))))))))
      )
  ...
  test("JSON.JObject, 1")(Gen.unit(())): _ =>
    assertEquals(
      ...
      Right(JObject(Map(
        ...
        "Related companies"  ->
          JArray(IndexedSeq(JString("HPQ"), JString("IBM"), JString("YHOO"), JString("DELL"), JString("GOOG")))
      )))
    )
